...
public class ServicioProductos implements IServicioProductos, Serializable {
	...
	public List<Producto> buscarProductos(String descripcion, String idCategoria, EstadoProducto estado,
		...
		if (idCategoria != null && !idCategoria.trim().isEmpty()) {
			...
			// Índice id -> Categoria
			java.util.List<segundum.modelo.Categoria> todas = repositorioCategorias.getAll();
			java.util.Map<String, segundum.modelo.Categoria> porId = new java.util.HashMap<>();
			for (segundum.modelo.Categoria c : todas) {
				porId.put(c.getId(), c);
			}

			segundum.modelo.Categoria raiz = porId.get(idCategoria);
			if (raiz != null) {
				// DFS por subcategorias
				java.util.Deque<segundum.modelo.Categoria> pila = new java.util.ArrayDeque<>();
				pila.push(raiz);
				while (!pila.isEmpty()) {
					segundum.modelo.Categoria actual = pila.pop();
					if (idsCategoriasValidas.add(actual.getId())) {
						java.util.List<segundum.modelo.Categoria> subs = actual.getSubcategorias();
						if (subs != null) {
							for (segundum.modelo.Categoria h : subs) {
								pila.push(h);
							}
						}
					}
